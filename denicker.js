...
class Denicker {
        ...
        try {
            ...
            if (KNOWN_NICK_SKINS.has(hash)) {
                ...
                if (this.api.config.get('showUnresolvedNicks.enabled')) {
                    this.sendAlert(teamFormattedName, null, name);
                }
            ...
            if (realName && realName !== name) {
                ...
                this.sendAlert(teamFormattedName, realName, name);
    ...
    sendAlert(playerName, realName, nickName = null) {
        ...
        const sendMessage = () => {
            if (realName && nickName) {
                const pasteMessage = `${nickName}-nicked ，他的 ign is: ${realName} 喵 ~`;
                const messageComponent = {
                    text: `${this.PLUGIN_PREFIX} `,
                    extra: [
                        {
                            text: `§6${realName}§7 is nicked as ${playerName}§7.`,
                            clickEvent: {
                ...
                const alertMsg = `${playerName}§7 is nicked.`;
